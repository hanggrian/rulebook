...
class TagDescriptionPunctuationRuleTest {
    private val assertThatCode = assertThatRule { TagDescriptionPunctuationRule() }
    ...
    fun `Tag description has no end punctuation`() = assertThatCode(
        ...
        LintViolation(2, 10, Messages.get(TagDescriptionPunctuationRule.MSG, "@param")),
        LintViolation(3, 10, Messages.get(TagDescriptionPunctuationRule.MSG, "@param"))
    )
    ...
    fun `Tag description ends with comments`() = assertThatCode(
        ...
        LintViolation(2, 10, Messages.get(TagDescriptionPunctuationRule.MSG, "@param")),
        LintViolation(4, 10, Messages.get(TagDescriptionPunctuationRule.MSG, "@param"))
    )
}
