...
import com.hendraanggrian.rulebook.codenarc.ReplaceWithAbstractCollectionNarc
import org.codenarc.rule.AbstractRuleTestCase
import kotlin.test.Test
import kotlin.test.assertEquals

class ReplaceWithAbstractCollectionNarcTest :
    AbstractRuleTestCase<ReplaceWithAbstractCollectionNarc>() {

    override fun createRule(): ReplaceWithAbstractCollectionNarc =
        ReplaceWithAbstractCollectionNarc()

    @Test
    fun `Rule properties`() {
        assertEquals(3, rule.priority)
        assertEquals("ReplaceWithAbstractCollection", rule.name)
    }
    ...
    fun `Use explicit collections as function parameter`() = assertViolations(
        ...
        mutableMapOf(
            ...
            "message" to Messages[ReplaceWithAbstractCollectionNarc.MSG_LIST]
        ),
        mutableMapOf(
            "line" to 2,
            "source" to "void hashSet(HashSet<Int> collection) {}",
            "message" to Messages[ReplaceWithAbstractCollectionNarc.MSG_SET]
        ),
        mutableMapOf(
            "line" to 3,
        ...
        mutableMapOf(
            ...
            "message" to Messages[ReplaceWithAbstractCollectionNarc.MSG_MAP]
        ),
        mutableMapOf(
            "line" to 5,
