...
import com.hendraanggrian.rulebook.codenarc.SourceAcronymCapitalizationVisitor.Companion.MSG
...
class SourceAcronymCapitalizationRuleTest :
    AbstractRuleTestCase<SourceAcronymCapitalizationRule>() {
    override fun createRule() = SourceAcronymCapitalizationRule()

    @Test
    fun `Rule properties`() {
        assertEquals(3, rule.priority)
        assertEquals("SourceAcronymCapitalization", rule.name)
    }
