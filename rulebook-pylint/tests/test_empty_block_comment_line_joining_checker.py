...
from rulebook_pylint.empty_block_comment_line_joining_checker \
    import EmptyBlockCommentLineJoiningChecker
...
class TestEmptyBlockCommentLineJoiningChecker(CheckerTestCase):
    ...
    def test_multiple_empty_lines_in_docstring(self):
        ...
        with self.assertAddsMessages(
            msg(EmptyBlockCommentLineJoiningChecker.MSG, (3, 4, 8, 7), node1.doc_node),
            msg(EmptyBlockCommentLineJoiningChecker.MSG, (12, 4, 17, 7), node2.doc_node),
        ):
            self.checker.visit_classdef(node1)
            self.checker.visit_functiondef(node2)
