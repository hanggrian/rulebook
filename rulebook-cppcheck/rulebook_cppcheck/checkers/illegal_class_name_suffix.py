...
class IllegalClassNameSuffixChecker(RulebookChecker):
    ...
    ID: str = 'illegal-class-name-suffix'
    MSG: str = 'illegal.class.name.suffix'
    ARG_ILLEGAL_CLASS_NAME_SUFFIXES: str = 'illegal-class-name-suffixes'
    ARGS = [ARG_ILLEGAL_CLASS_NAME_SUFFIXES]
    ...
    def before_run(self, args: dict[str, str]) -> None:
        self._illegal_class_name_suffixes = \
            set(args[self.ARG_ILLEGAL_CLASS_NAME_SUFFIXES].split(','))
